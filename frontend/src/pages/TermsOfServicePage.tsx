...
import { useTerms, TermContentBlock } from "../hooks/useTerms";
import { PortableText, PortableTextComponents } from "@portabletext/react";
import { PortableTextBlock } from "@portabletext/types";

// Convert Strapi content to PortableText format
function formatContent(blocks: TermContentBlock[] | undefined): PortableTextBlock[] {
  if (!blocks) return [];
...
// Define Tailwind-styled components for PortableText
...
export default function TermsOfServicePage() {
  const { data: titleData, loading: titleLoading, error: titleError } = usePageTitle("terms-and-conditions");
  const { data: contentData, loading: contentLoading, error: contentError } = useTerms();
  ...
  const formattedContent = formatContent(contentData?.content);
